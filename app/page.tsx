...
export default function Home(){
 ...
 return <main className="wrap"><div className="top"><div><div className="brand">Famiglia Grassi</div><div className="muted">Area familiare privata</div></div><button className="btn secondary" onClick={async()=>{await supabase().auth.signOut();setUser(null)}}>Esci</button></div><div className="grid">{sections.map(([label,table,desc])=><a className="card" key={table} href={'./area/?table='+table}><span className="pill">Tabella</span><h3>{label}</h3><div className="muted">{desc}</div></a>)}</div></main>
}
...
function Login({onLoggedIn}:{onLoggedIn:(user:any)=>void}){
 ...
 async function go(e:any){
   ...
   if(mode==='register'){
     ...
     if(data.session?.user){onLoggedIn(data.session.user);setBusy(false);return;}
     ...
     if(error){setError(error.message);} else if(data.user){onLoggedIn(data.user);}
   }
   setBusy(false);
 }
